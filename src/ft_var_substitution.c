...
static char *process_dollar(t_list *gc, char *input, size_t *i, char **env, char *result, size_t *result_len);
static void append_until_dollar(t_list *gc, char *input, size_t *i, char *result, size_t *result_len);

char *substitute_variables(t_list *gc, char *input, char **env)
{
    char *result = gc_malloc(gc, 1, strlen(input) + 1);
    size_t result_len = 0;
    size_t i = 0;
...
{
    size_t start = *i;
    size_t next_dollar = strlen_char_quoted(input + start, '$');

    if (next_dollar > start)
    {
        size_t chunk_len = next_dollar - start;
...
static char *process_dollar(t_list *gc, char *input, size_t *i, char **env, char *result, size_t *result_len)
{
    ...
    size_t var_len = ft_varlen(input + var_start);

    if (var_len > 0)
    {
        char *var_name = ft_strndup(input + var_start, var_len);
